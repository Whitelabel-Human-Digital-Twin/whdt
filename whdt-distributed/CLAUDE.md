...
- `whdt-core` (`implementation`, not `api`) — provides domain types as SerDe targets; consumers must declare their own `whdt-core` dependency

## Consumed by

- `whdt-wldt-plugin` — uses `Stub` SerDe instances and `Namespace` for MQTT adapter wiring
