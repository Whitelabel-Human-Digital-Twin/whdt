...
- `whdt-distributed` — builds `SerDe` instances over core types for MQTT messaging
- `whdt-wldt-plugin` — converts `HumanDigitalTwin` descriptors to runnable WLDT instances; provides the `App` implementation as `WldtApp`
- `whdt-examples` — constructs HDT descriptors directly
